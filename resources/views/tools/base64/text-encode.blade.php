...
<div class="grid gap-6 md:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
    <div class="glass-card rounded-2xl border border-white/10 bg-slate-900/60 p-6">
        ...
        <p class="text-sm text-slate-400 mb-4">
            Paste any text below to convert it into a Base64 string using the external API.
        </p>

        @include('tools.base64.partials._request_form', ['submitLabel' => 'Encode text'])
    </div>
    ...
    <div class="glass-card rounded-2xl border border-white/10 bg-slate-900/60 p-6">
        ...
        'emptyMessage' => 'Submit some text to see the encoded Base64 output as returned by the API.',
        ])
    </div>
</div>
@endsection